...
func (engine *HTMLRenderingEngine) NeedsRender(section *booklit.Section, path string) bool {
	if section.ModTime.IsZero() {
		return true
	}
	...
	if section.ModTime.After(info.ModTime()) {
		return true
	}

	for _, t := range engine.tmplModTimes {
